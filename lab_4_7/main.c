...
state init_string(char** str, int* buf, int* size);
...
state parsing_file(FILE* file, memory* storage)
{
    ...
    while(ch != EOF)
    {
        ...
        char* var_name = (char*)malloc(sizeof(char)*3);
        if(var_name == NULL)
        {
            return meme_problem;
        }
        ...
        operation[0] = '\0';
        char* first_arg = (char*)malloc(sizeof(char)*3);
        if(first_arg == NULL)
        {
            free(var_name);
            free(operation);
        ...
        first_arg[0] = '\0';
        char* second_arg = (char*)malloc(sizeof(char)*3);
        if(second_arg == NULL)
        {
            free(var_name);
            free(operation);
...
state get_instruction(FILE* file, char** name, char** operation, char** first_arg, char** second_arg)
{
    int name_size = 0, name_buf = 2, operation_size = 0, operation_buf = 2, first_arg_size = 0, first_arg_buf = 2, second_arg_size = 0, second_arg_buf = 2;
    state st = well;
    int ch;
//    st = init_string(name, &name_buf, &name_size);
//    if(st != well)
//        return st;
//    *name = (char*)malloc(sizeof(char)*(name_buf + 1));
//    if(*name == NULL)
//        return meme_problem;
//
//    st = init_string(operation, &operation_buf, &operation_size);
//    if(st != well) {
//        free_4_arr(first_arg, second_arg, operation, name);
//        return st;
//    }
//    st = init_string(first_arg, &first_arg_buf, &first_arg_size);
//    if(st != well) {
//        free_4_arr(first_arg, second_arg, operation, name);
//        return st;
//    }
//    st = init_string(second_arg, &second_arg_buf, &second_arg_size);
//    if(st != well) {
//        free_4_arr(first_arg, second_arg, operation, name);
//        return st;
//    }
    ...
    else
    {
        if(strcmp(*name, "print") == 0)
        {
            ...
            while(ch != ' ' && ch != ';' && ch != '\n' && ch != '\t' && ch != '\r')///////////////////////////////////////////////TODO
            {
                ...
                if(is_operation((char)ch) && !isalpha(ch) && !isdigit(ch))
                {
                    free_4_arr(first_arg, second_arg, operation, name);
                    return invalid_input;
                }
...
state init_string(char** str, int* buf, int* size)
{
    ...
    *str = (char*)malloc(sizeof(char)*(*buf + 1));
    if(*str == NULL) return meme_problem;
    return well;
}
